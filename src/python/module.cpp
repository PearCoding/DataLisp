...
template<class T>
void export_list(const char* typeName)
{
    bpy::class_<std::list<T> >(typeName)
        .def("__len__", &std::list<T>::size)
        .def("clear", &std::list<T>::clear)
        .def("append", &listwrap<T>::add,
            bpy::with_custodian_and_ward<1,2>()) // to let container keep value
        .def("__getitem__", &listwrap<T>::get,
            bpy::return_value_policy<bpy::copy_non_const_reference>())
        .def("__setitem__", &listwrap<T>::set,
            bpy::with_custodian_and_ward<1,2>()) // to let container keep value
        .def("__delitem__", &listwrap<T>::del)
        .def("__contains__", &listwrap<T>::in)
        .def("__iter__", bpy::iterator<std::list<T> >())
...
BOOST_PYTHON_MODULE(pydatalisp)
{
    ...
    bpy::class_<Data_PY>("Data",
        ...
        .add_property("floatConverted", &Data_PY::getFloatConverted)
